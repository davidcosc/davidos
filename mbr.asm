...
main:    
  ...
  call print_string                        ; a call is a jump that additionally pushes the return address (address of the next instruction) to the stack
    
loop:                                      ; jumping to a label is done using a relative jump which means we do not need to know the specific address referenced by the loop label
  jmp loop                                 ; instead begining at the byte immediatley after the jmp instruction, we calculate the distance in bytes to the label 
